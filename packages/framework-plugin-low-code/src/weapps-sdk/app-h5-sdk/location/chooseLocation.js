...
class LocationChooser extends Component {
  ...
  render () {
    return createPortal(
      <div className='weapps_chooselocation' ref={this.getWrapRef}>
        <div className='weapps_chooselocation_bar'>
          <div className='weapps_chooselocation_back' onClick={this.onBack} />
          <p className='weapps_chooselocation_title'>返回</p>
          <button className='weapps_chooselocation_submit' onClick={this.onSubmit}>完成</button>
        </div>
        <iframe className='weapps_chooselocation_frame' frameborder='0' src={`https://apis.map.qq.com/tools/locpicker?search=1&type=1&key=${LOCATION_APIKEY}&referer=myapp`} />
      </div>,
      document.body
    )
  }
...
const chooseLocation = ({ success, fail, complete } = {}) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    ReactDom.render(<LocationChooser handler={res => {
      if (res) {
        onError(res)
      } else {
