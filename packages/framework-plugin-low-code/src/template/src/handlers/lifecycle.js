...
export function initLifeCycle({
  ...
  if (!window.$$global.alreadyInitAppLifeCycle) {
    if (!window.$$global.isOnLaunchRunned) {
      onAppLaunch(wx.getLaunchOptionsSync())
  ...
  // wx兼容的页面级别事件监听
  if (process.env.isMiniprogram) {
    window.addEventListener('wxload', (...args) => onPageLoad(...args))
...
export function pageLifeCycleMount(
  ...
  let query = location.hash.split("?")[1] ? querystring.parse(location.hash.split("?")[1]): {}
  typeof onPageLoad === 'function' && onPageLoad(query)
  useEffect(() => {
    
    typeof onPageReady === 'function' && onPageReady(query)
