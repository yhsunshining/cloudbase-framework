...
export async function fixAppJson(appBuildDir: string) {
  ...
  appJson.subpackages = appJson.subpackages.map(config => {
    config.pages = config.pages.map(pagePath => {
...
export async function generateKboneTabBarConfig(mpConfig: Record<string, any>, appBuildDir) {
  ...
  if (mpConfig.appExtraConfig.tabBar) {
    if (mpConfig.appExtraConfig.tabBar.list) {
      await Promise.all(
        mpConfig.appExtraConfig.tabBar.list.map(async item => {
          if (item.iconPath) {
...
export function getWebpackWebBuildParams(
  ...
  if (buildTypeList.includes('qywxH5')) {
    jsApis = ['//res.wx.qq.com/open/js/jweixin-1.2.0.js']
  } else if (buildTypeList.includes('wxH5')) {
  ...
  return {
    ...
    output: {
      ...
      publicPath: buildTypeList.includes('app') ? '' : publicPath,
      pathinfo: false,
    },
    ...
    definePlugin: {
      ...
      'process.env.isApp': buildTypeList.includes('app'), // 注入环境变量，注入isApp
    },
  } as any
}
...
export async function downloadDependencies(targetDir: string, srcZipUrl: string) {
  ...
  if (err) {
    console.error('Fail to download weapps material package ' + srcZipUrl, err)
    throw err
  }
