...
export function generateDataBind(bind: IDynamicValue) {
  ...
  if (type === PropBindType.forItem) {
    ...
    expr = `${value.replace(/\$comp./g, `${COMPONENT_API_PREFIX}.`)}`;
  } else if (type === PropBindType.prop) {
    const isNegated = value.startsWith('!');
    const originValue = value.replace(/^!/, '');
    expr = `${
      isNegated ? '!' : ''
    }${COMPONENT_API_PREFIX}.props.data.${originValue}`;
  } else {
    const isGlobal = value.startsWith('global.');
    const isCopositive =
      value.startsWith('comp-') || value.startsWith('$comp_');
    ...
    switch (type) {
      ...
      case PropBindType.paramData: {
        if (isCopositive) {
          expr = value
            .replace(/^comp-\w+/, `${COMPONENT_API_PREFIX}.${PREFIX_MAP[type]}`)
            .replace(
              /^\$comp_\w+/,
              `${COMPONENT_API_PREFIX}.${PREFIX_MAP[type]}`
            );
        } else {
          expr = value
            .replace(
...
export function createEventHanlders(
  ...
  walkThroughWidgets(widgets, (id, widget, parentId) => {
    const { xComponent } = widget;
    const xProps: Required<IWeAppComponentInstance>['xProps'] =
      widget.xProps || ({} as any);
    if (!xComponent) {
      // skip slot component
      return;
    }
