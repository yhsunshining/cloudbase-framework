...
<% materials.map(material => { if(material.entry){ %>
import <%= _.camelCase(material.name)%> from 'libraries/<%=material.name%>@<%= material.version %>/<%=material.entry%>';
const <%= _.camelCase(material.name)%>Actions = <%= _.camelCase(material.name)%>.actions
